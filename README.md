...
AI Copilot is under active development. The core is stable and improvements ship regularly.
...
- **setup.sh will not run.** Make sure you are in the project folder (`cd AI Copilot`) and that the script is executable (`chmod +x setup.sh`). On Windows, use Git Bash.
...
AI Copilot collects no data and sends no telemetry. Processing happens locally, and your session stays on your device. Requests to the AI provider are encrypted in transit.
