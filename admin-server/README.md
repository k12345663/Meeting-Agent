...
Render.com, since it supports a **persistent disk** (without one, most
hosting platforms wipe the SQLite file on every redeploy, resetting the
admin list and all settings):

1. Push this repo to GitHub (if it isn't already).
2. In Render: **New → Blueprint**, point it at the repo. Render reads
   [`render.yaml`](render.yaml) and provisions the service + a 1GB
